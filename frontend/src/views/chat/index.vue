  ...
  >
    <div ref="scrollContainer" class="chat_scroll_box" @scroll="handleScroll">
      <div class="msg_list" :class="{ 'is-embedded': embeddedMode }">
        ...
        <div
          v-if="messagesList.length === 0 && !loading"
          class="suggested-questions-container"
          :class="{
            'has-questions':
              suggestedQuestions.length > 0 || suggestedQuestionsLoading,
          }"
        >
          <!-- 骨架屏占位 -->
          <div
            v-if="suggestedQuestionsLoading && suggestedQuestions.length === 0"
            class="suggested-questions-inner"
          >
            <div class="suggested-questions-title">
              <t-skeleton
                animation="gradient"
                :row-col="[{ width: '120px', height: '18px' }]"
              />
            </div>
          ...
          <transition v-else appear name="sq-fade">
            ...
            >
              ...
              <div class="suggested-questions-grid">
                ...
                >
                  <span class="suggested-question-text">{{
                    item.question
                  }}</span>
                  <span
                    v-if="item.source === 'faq'"
                    class="suggested-question-badge faq"
                    >FAQ</span
                  >
                </div>
              </div>
...
const fetchSuggestedQuestions = async () => {
  ...
  try {
    ...
    if (!agentId) return;
    const selectedKBs = props.embeddedMode
      ? props.kbIds
      : useSettingsStoreInstance.getSelectedKnowledgeBases();
    const selectedFiles = props.embeddedMode
      ? []
      : useSettingsStoreInstance.getSelectedFiles();
...
const debouncedFetchSuggestions = () => {
  ...
  suggestedDebounceTimer = setTimeout(() => {
...
const onChatScrollTop = () => {
  if (scrollLock.value || historyLoadingMore.value || !hasMoreHistory.value)
    return;
...
const reconstructEventStreamFromSteps = (
  ...
  if (agentSteps && Array.isArray(agentSteps) && agentSteps.length > 0) {
    agentSteps.forEach((step) => {
      ...
      const stepTimestamp = step.timestamp
...
const handleMsgList = async (data, isScrollType = false, newScrollHeight) => {
  ...
  for (let i = 0, len = chatlist.length; i < len; i++) {
    ...
    if (willContinueStream) {
      ...
      item.agent_steps = item.agent_steps
        ? markRaw(item.agent_steps)
        : item.agent_steps;
    ...
    if (item.content) {
      if (
        ...
        item.thinkContent = item.content
          .trim()
          .substring(0, index)
          .replace("<think>", "")
          .trim();
  ...
  if (
    ...
    !messagesList[messagesList.length - 1].is_completed
  ) {
    ...
    if (lastMessage.role === "assistant") {
      ...
      console.log(
        "[Continue Stream] Set assistant message ID:",
        lastMessage.id,
      );
    }
    await startStream({
...
const sendMsg = async (
  ...
  const agentEnabled = props.embeddedMode
    ...
    : useSettingsStoreInstance.isAgentEnabled;

  // Get web search status from settings store
  const webSearchEnabled = props.embeddedMode
    ? false
  ...
  const mcpServiceIds = props.embeddedMode
    ? []
    : useSettingsStoreInstance.settings.selectedMCPServices || [];
...
onChunk((data) => {
  ...
  if (data.response_type === "agent_query") {
    if (data.assistant_message_id) {
      ...
      console.log(
        "[Agent Query] Saved assistant message ID:",
    ...
    // 检查是否是继续流式传输（消息已存在）
    const existingMessage = findLastMessage(
    ...
    if (!existingMessage) {
      ...
      console.log(
        "[Agent Query] Continuing stream for existing message, keeping current loading state",
      );
    }
    return;
  }
  ...
  if (data.response_type === "references") {
    ...
    console.log(
      "[References] Saved to message, count:",
      existingMessage.knowledge_references.length,
    );
    return;
  }
  ...
  if (existingMessage?.is_completed && data.done && !data.content) {
    console.log(
      "[Non-Agent] Ignoring duplicate completion event for completed message",
    );
    return;
  }
  ...
  if (
    fullContent.value.includes("<think>") &&
    !fullContent.value.includes("<\/think>")
  ) {
    obj.thinking = true;
    obj.showThink = true;
    ...
    obj.thinkContent = fullContent.value
      .substring(0, index)
      .replace("<think>", "")
      .trim();
  ...
  switch (data.response_type) {
    case "thinking":
      {
        ...
        if (!data.done) {
          ...
          if (!thinkingEvent) {
            ...
            console.log(
              "[Thinking] Creating new thinking event, event_id:",
              eventId,
            );
            thinkingEvent = {
              type: "thinking",
          ...
          if (thinkingEvent) {
            console.log(
              "[Thinking] Completing event, event_id:",
              eventId,
              "content length:",
              thinkingEvent.content.length,
            );
            ...
            thinkingEvent.duration_ms =
              data.data?.duration_ms || Date.now() - thinkingEvent.startTime;
            thinkingEvent.completed_at = data.data?.completed_at || Date.now();

            console.log(
              "[Thinking] Event completed, duration:",
              thinkingEvent.duration_ms,
              "ms",
            );
          } else {
            console.warn(
              "[Thinking] Received done for unknown event_id:",
              eventId,
            );
          }
        }
    ...
    case "tool_call":
      ...
      if (data.data && (data.data.tool_name || data.data.tool_call_id)) {
        ...
        if (!toolCallId) {
          console.warn(
            "[Tool Call] Received event without identifiable tool_call_id:",
            data.data,
          );
          break;
        }
        ...
        if (!toolCallEvent) {
          toolCallEvent = message.agentEventStream.find(
            (event) =>
              event.type === "tool_call" && event.tool_call_id === toolCallId,
          );
        }
    ...
    case "error":
      ...
      if (data.data) {
        ...
        const success =
          data.response_type !== "error" && data.data.success !== false;
        ...
        if (toolCallEvent) {
          ...
          toolCallEvent.output = success
            ? data.data.output || data.content
            : data.data.error || data.content;
          toolCallEvent.error = !success
            ? data.data.error || data.content
            : undefined;
          // Set both duration and duration_ms for compatibility
          const duration =
            data.data.duration_ms !== undefined
          ...
          console.warn(
            "[Tool Result] No pending tool call found for",
            toolCallId || toolName,
          );
        }
    ...
    case "answer":
      ...
      if (data.content) {
        ...
        fullContent.value += data.content;
        console.log(
          "[Answer] Content appended, new length:",
      ...
      let answerEvent = message.agentEventStream.find(
        (e) => e.type === "answer",
      );
      if (!answerEvent) {
        answerEvent = {
      ...
      if (data.content) {
        ...
        console.log(
          "[Answer] answerEvent.content updated, length:",
          answerEvent.content.length,
        );
      }
      ...
      if (data.done && !answerEvent.done) {
        ...
        console.log(
          "[Agent] Answer done, content length:",
          message.content?.length || 0,
          "answerEvent.content length:",
          answerEvent.content?.length || 0,
        );
        ...
        console.log(
          "[Answer] Ignoring duplicate done event, current content preserved:",
          answerEvent.content?.length || 0,
        );
      }
      break;
...
const updateAssistantSession = (payload) => {
  ...
  if (message) {
    ...
    message.knowledge_references = message.knowledge_references
...
onMounted(async () => {
  ...
  const agentIdFromQuery =
    props.embeddedAgentId ||
    (route.query.agent_id && String(route.query.agent_id));
  const sourceTenantIdFromQuery =
    route.query.source_tenant_id && String(route.query.source_tenant_id);
  if (agentIdFromQuery && sourceTenantIdFromQuery) {
    useSettingsStoreInstance.selectAgent(
      agentIdFromQuery,
      sourceTenantIdFromQuery,
    );
...
.chat.is-mobile .chat_scroll_box {
  padding: 0 12px;
}

.chat.is-mobile .input-container {
  margin: 8px auto 0;
  padding: 0 12px;
  min-height: auto;
  max-width: 100%;
...
.chat.is-mobile .msg_list {
  max-width: 100%;
  gap: 12px;
}

.chat.is-mobile .suggested-questions-container {
  padding: 20px 12px 12px;
}

.chat.is-mobile .suggested-question-card {
  padding: 8px 14px;
}

.chat.is-mobile .suggested-question-text {
  font-size: 12px;
}

.chat.is-mobile .scroll-to-bottom-btn {
  bottom: 120px;
}
