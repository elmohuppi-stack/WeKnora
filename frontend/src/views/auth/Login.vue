      ...
      <div class="language-switch">
        ...
        <div v-if="showLanguageMenu" class="language-dropdown">
          ...
          >
            ...
            <span v-if="currentLanguage === lang.value" class="check-icon"
              >✓</span
            >
          </div>
        </div>
        ...
        <div class="form-card" v-if="!isRegisterMode">
          ...
          <div class="form-content">
            ...
            >
              ...
              <div
                class="form-footer login-form-footer"
            ...
            <div class="login-features">
              <div class="feature-item">
                ...
                <span class="feature-text">{{
                  $t("platform.multimodalParsing")
                }}</span>
              </div>
              <div class="feature-item">
                <span class="feature-icon">✓</span>
                <span class="feature-text">{{
                  $t("platform.hybridSearchEngine")
                }}</span>
              </div>
              <div class="feature-item">
                <span class="feature-icon">✓</span>
          ...
          <div class="form-content">
            ...
            <div class="login-features">
              <div class="feature-item">
                <span class="feature-icon">✓</span>
                <span class="feature-text">{{
                  $t("platform.independentTenant")
                }}</span>
              </div>
              <div class="feature-item">
                <span class="feature-icon">✓</span>
                <span class="feature-text">{{
                  $t("platform.fullApiAccess")
                }}</span>
              </div>
              <div class="feature-item">
                <span class="feature-icon">✓</span>
                <span class="feature-text">{{
                  $t("platform.knowledgeBaseManagement")
                }}</span>
              </div>
            </div>
          </div>
...
const currentLangOption = computed(() =>
  languageOptions.find((l) => l.value === currentLanguage.value),
);
...
const persistLoginResponse = async (response: any) => {
  ...
  //
  // When the backend omits active_tenant/tenant entirely (some deployments
  // only return memberships), fall back to user.tenant_id so the store
  // still gets populated and the redirect works.
  const activeTenant =
    response.active_tenant ||
    response.tenant ||
    (response.memberships?.[0]
      ? {
          id: response.memberships[0].tenant_id,
          name: response.memberships[0].tenant_name || "",
        }
      : response.user?.tenant_id
        ? { id: response.user.tenant_id, name: "" }
        : null);
